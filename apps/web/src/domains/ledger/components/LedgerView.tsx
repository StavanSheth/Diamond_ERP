...
import { ItemDetailDrawer } from '../../inventory/components/ItemDetailDrawer';
import { AddTransactionButton } from '../../common/components/buttons/AddTransactionButton';
...
import { EntityPreloader } from '../../common/components/EntityPreloader';
...
export const LedgerView: React.FC<LedgerViewProps> = ({ 
  ...
  useEffect(() => {
    api.getLedgerStocks()
      ...
      .catch((err) => console.error('Failed to load ledger stocks:', err));
  }, []);
  ...
  const summary = useMemo(() => {
    ...
    const totalBrokerage = entries.reduce((s, e) => s + Number(e.brokerageAmount || 0), 0);

    return { totalIn, totalOut, valueIn, valueOut, totalItems, txnCount: entries.length, finalBalanceCarat, finalBalanceValue, totalPaymentDone, totalPaymentDue, payableDue, payablePaid, receivableDue, receivableCollected, totalBrokerage };
  }, [entries]);
      ...
      <header className="px-margin-page py-lg bg-surface border-b border-outline-variant shrink-0">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-gutter">
          <div>
            ...
            <h2 className="font-headline-lg text-headline-lg text-on-surface">
              {title || (certificateId ? 'Certificate Ledger' : repairId ? 'Repair Ledger' : partyId ? 'Party Ledger' : itemCode ? 'Item Ledger' : selectedStock ? 'Stock Ledger' : 'Global Ledger')}
            </h2>
          </div>
          ...
          <div className="flex items-center gap-md flex-wrap mt-md md:mt-0">
            ...
            <button
              onClick={async () => {
                setExporting(true);
                try {
                  const filename = `Ledger_Statement_${new Date().toISOString().split('T')[0]}.xlsx`;
                  await api.downloadReportExcel({
                    reportType: 'GENERAL_LEDGER',
                    stockId: selectedStock || undefined,
              ...
              disabled={exporting}
              title="Export Ledger with Debit, Credit & Closing Balance"
              className="flex items-center gap-xs bg-surface-container hover:bg-surface-container-high text-on-surface border border-outline-variant rounded-md px-md py-sm font-body-md font-bold transition-colors shadow-sm disabled:opacity-50"
            >
              <span className="material-symbols-outlined text-[18px] text-primary">download</span>
              <span>{exporting ? 'Exporting...' : 'Export Ledger'}</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-xl mt-md flex-wrap">
          <div className="flex items-center gap-xs">
            ...
            <span className="font-caption text-caption text-on-surface-variant font-bold">DEBIT (DR / IN):</span>
            <span className="font-kpi-numeric text-[16px] text-[#2E7D32] tabular-nums font-bold">{summary.totalIn.toFixed(2)} ct</span>
          ...
          <div className="flex items-center gap-xs">
            ...
            <span className="font-caption text-caption text-on-surface-variant font-bold">CREDIT (CR / OUT):</span>
            <span className="font-kpi-numeric text-[16px] text-[#D32F2F] tabular-nums font-bold">{summary.totalOut.toFixed(2)} ct</span>
            <span className="font-caption text-caption text-outline ml-xs">({fmt(summary.valueOut)})</span>
          </div>
          <div className="h-4 w-px bg-outline-variant" />
          <div className="flex items-center gap-xs bg-surface-container-low px-sm py-xs rounded border border-outline-variant/40">
            <span className="material-symbols-outlined text-[18px] text-primary">account_balance_wallet</span>
            <span className="font-caption text-caption text-on-surface-variant font-bold">CLOSING BALANCE:</span>
            <span className="font-kpi-numeric text-[16px] text-primary tabular-nums font-bold">{(summary.finalBalanceCarat || 0).toFixed(2)} ct</span>
            <span className="font-caption text-caption text-primary ml-xs font-semibold">({fmt(summary.finalBalanceValue || 0)})</span>
          </div>
          <div className="h-4 w-px bg-outline-variant" />
          <span className="font-caption text-caption text-on-surface-variant font-medium">{summary.txnCount} transactions</span>
          <div className="h-4 w-px bg-outline-variant" />
          <div className="flex items-center gap-xs">
            <span className="font-caption text-caption text-on-surface-variant font-bold">TO PAY:</span>
            <span className="font-kpi-numeric text-[16px] text-[#E65100] tabular-nums font-bold">{fmt(summary.payableDue)}</span>
            <span className="font-caption text-caption text-outline ml-xs">(Paid: {fmt(summary.payablePaid)})</span>
          </div>
          <div className="h-4 w-px bg-outline-variant" />
          <div className="flex items-center gap-xs">
            <span className="font-caption text-caption text-on-surface-variant font-bold">TO COLLECT:</span>
            <span className="font-kpi-numeric text-[16px] text-[#1565C0] tabular-nums font-bold">{fmt(summary.receivableDue)}</span>
          ...
          {summary.totalBrokerage > 0 && (
            <>
              <div className="h-4 w-px bg-outline-variant" />
              <div className="flex items-center gap-xs bg-amber-50/80 px-sm py-xs rounded border border-amber-200/60">
                <span className="material-symbols-outlined text-[18px] text-[#795548]">handshake</span>
                <span className="font-caption text-caption text-[#5D4037] font-bold">TOTAL BROKERAGE:</span>
                <span className="font-kpi-numeric text-[16px] text-[#795548] tabular-nums font-bold">{fmt(summary.totalBrokerage)}</span>
              </div>
            </>
          )}
        </div>
      ...
      <div className="flex-1 overflow-auto">
        {loading && (
          <div className="p-margin-page">
            <EntityPreloader viewMode="table" count={8} tableColumns={10} />
          </div>
        )}
          ...
          <table className="w-full text-left border-collapse min-w-[950px]">
            <thead className="bg-surface-container-low text-on-surface-variant font-caption text-caption font-bold sticky top-0 z-20 border-b-2 border-outline-variant shadow-sm">
              <tr>
                ...
                <th className="py-sm px-md text-right text-[#2E7D32]">Debit (Dr / In)</th>
                <th className="py-sm px-md text-right text-[#D32F2F]">Credit (Cr / Out)</th>
                <th className="py-sm px-md text-right bg-surface-container border-l border-outline-variant font-bold text-primary">Closing Bal. (Ct)</th>
                <th className="py-sm px-md text-right bg-surface-container font-bold text-primary">Closing Bal. (₹)</th>
                <th className="py-sm px-md text-right font-bold">Total (₹)</th>
                <th className="py-sm px-md text-right font-bold text-[#795548]">Brokerage</th>
                  ...
                  <React.Fragment key={entry.id}>
                    <tr className={`hover:bg-surface-container-low transition-colors cursor-pointer`} onClick={() => toggleRow(entry.id)}>
                      ...
                      <td className="px-md py-sm whitespace-nowrap">
                        <div className="flex items-center gap-1.5 flex-wrap max-w-[200px]">
                          <span className="font-medium text-on-surface truncate">{entry.party?.name || '—'}</span>
                          {entry.party?.partyType && (
                            <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold border ${getPartyTypeConfig(entry.party.partyType).badgeClass}`}>
                              {getPartyTypeConfig(entry.party.partyType).badgeText}
                            </span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-md py-sm text-right whitespace-nowrap">
                        {Number(entry.brokerageAmount || 0) > 0 ? (
                          <div className="flex flex-col items-end">
                            <div className="flex items-center gap-1">
                              <span className="font-bold text-[#795548]">{fmt(entry.brokerageAmount)}</span>
                              <span className={`px-1 py-0.2 text-[9px] font-bold rounded ${
                                entry.brokerageType === 'EXCLUSIVE'
                                  ? 'bg-amber-100 text-amber-800 border border-amber-200'
                                  : 'bg-teal-50 text-teal-700 border border-teal-200'
                              }`}>
                                {entry.brokerageType === 'EXCLUSIVE' ? 'Excl.' : 'Incl.'}
                              </span>
                            </div>
                            {Number(entry.brokeragePercentage || 0) > 0 && (
                              <span className="text-[10px] font-semibold text-outline">
                                ({Number(entry.brokeragePercentage).toFixed(2)}%)
                              </span>
                            )}
                          </div>
                        ) : Number(entry.brokeragePercentage || 0) > 0 ? (
                    ...
                    {isExpanded && (
                      <tr className="bg-surface-container-lowest">
                        <td colSpan={13} className="p-0 border-b-2 border-outline-variant">
            ...
            <tfoot className="bg-surface-container-high text-on-surface font-bold sticky bottom-0 border-t-2 border-outline-variant shadow-sm text-body-md tabular-nums z-20">
              <tr className="hover:bg-surface-container-highest transition-colors">
                ...
                <td className="py-md px-md text-right font-bold text-[#795548]">
                  {summary.totalBrokerage > 0 ? fmt(summary.totalBrokerage) : '—'}
                </td>
                <td></td>
              </tr>
              {showAllItems && (
                <tr className="bg-surface-container-lowest">
                  <td colSpan={13} className="p-0 border-t-2 border-outline-variant">
