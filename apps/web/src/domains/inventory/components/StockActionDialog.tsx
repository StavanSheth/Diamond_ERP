...
interface StockActionDialogProps {
  ...
  onToggleArchive?: (stock: StockItem) => void;
}
      ...
      >
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="font-caption text-caption text-on-surface-variant">Update identity, location, remarks &amp; status</p>
          </div>
        ...
        {onToggleArchive && (
          <button
            onClick={() => {
              onClose();
              onToggleArchive(stock);
            }}
            className="w-full flex items-center gap-md p-md rounded-lg border border-outline-variant hover:bg-surface-container hover:border-primary transition-all group text-left"
          >
            <span className={`material-symbols-outlined transition-colors ${stock.status === 'ARCHIVED' ? 'text-primary' : 'text-outline-variant group-hover:text-primary'}`}>
              {stock.status === 'ARCHIVED' ? 'unarchive' : 'archive'}
            </span>
            <div className="flex-1">
              <h4 className="font-body-lg font-bold text-on-surface group-hover:text-primary transition-colors">
                {stock.status === 'ARCHIVED' ? 'Restore Stock to Active' : 'Archive Stock'}
              </h4>
              <p className="font-caption text-caption text-on-surface-variant">
                {stock.status === 'ARCHIVED' ? 'Reactivate and show in normal inventory' : 'Deactivate and mark as Archived'}
              </p>
            </div>
            <span className="material-symbols-outlined text-outline-variant">chevron_right</span>
          </button>
