...
export const OnboardingWizard: React.FC = () => {
  ...
  const fetchStatus = async () => {
    try {
      ...
      if (data.lifecycleState === 'USER_DISCOVERY') {
        ...
        if (dbs.candidates.length > 0) {
          setCandidatePath(dbs.candidates[0].canonicalPath);
        }
      }
    } catch (err: any) {
  ...
  const handleInspect = async () => {
    ...
    setError(null);
    try {
      const preview = await api.onboarding.inspectDatabase(candidatePath.trim());
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-[9998] flex items-center justify-center bg-slate-950/95 backdrop-blur-xl p-4 select-none animate-fadeIn text-slate-100"
    >
      <div className="w-full max-w-xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden p-8 relative">
        ...
        {currentStep === 'PIN_SETUP' && (
          <form onSubmit={handlePinSetup} className="space-y-5">
            <div className="text-sm text-slate-300">
              Set up a 6-digit application PIN to protect your local workstation lock and device operations.
            </div>
            <div>
            ...
            >
              {submitting ? 'Saving PIN...' : 'Save & Continue'}
            </button>
          </form>
        )}
        ...
        {currentStep === 'DEVICE_SETUP' && (
          <form onSubmit={handleDeviceSetup} className="space-y-5">
            <div className="text-sm text-slate-300">
              Register this computer as a trusted application terminal.
            </div>
            <div>
              ...
              <input
                ...
                placeholder="e.g. Surat Main Terminal"
                className="w-full px-4 py-3 bg-slate-950/60 border border-slate-700 rounded-xl text-slate-100 focus:ring-2 focus:ring-indigo-500"
              />
            </div>
            <button
        ...
        {currentStep === 'USER_DISCOVERY' && (
          <form onSubmit={handleUserStep} className="space-y-5">
            <div className="text-sm text-slate-300">
              Choose an existing ERP business account or create a new administrator account.
            </div>
            ...
            {dbMode === 'NEW' ? (
              <form onSubmit={handleCreateNewDb} className="space-y-4">
                ...
                <button
                  ...
                  className="w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-xl shadow-lg transition-all disabled:opacity-50"
                >
                  {submitting ? 'Creating Database...' : 'Provision & Attach Database'}
                </button>
              </form>
            ) : (
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="C:\...\data.db"
                      className="flex-1 px-4 py-3 bg-slate-950/60 border border-slate-700 rounded-xl text-slate-100"
                    />
                    <button
                      type="button"
                      onClick={handleInspect}
                      disabled={submitting || !candidatePath.trim()}
                      className="px-4 py-3 bg-slate-800 hover:bg-slate-700 rounded-xl text-sm font-medium"
                    >
                      Inspect
                    </button>
                  </div>
                ...
                {inspectPreview && (
                  <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-2 text-sm">
                    <div className="font-semibold text-white">Database Inspection Preview:</div>
                    <div>Status: <span className="text-indigo-400">{inspectPreview.status}</span></div>
                    <div>Suitability: <span className="text-amber-400">{inspectPreview.suitability}</span></div>
                    <div>Tables Found: <span className="text-slate-300">{inspectPreview.tableCount}</span></div>
                    {inspectPreview.details && (
                      <div className="text-xs text-slate-400">{inspectPreview.details}</div>
                    )}
                    ...
                    <div className="pt-3 border-t border-slate-800">
                      <label className="flex items-center gap-2 cursor-pointer text-xs text-slate-300">
                        ...
                        />
                        <span>I confirm attaching this existing database to this installation.</span>
                      </label>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full mt-3 py-3 px-4 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-xl shadow-lg transition-all disabled:opacity-50"
                    >
