...
export class VersionService {
  ...
  async createVersion(params: CreateVersionParams) {
    ...
    const latestVersion = await prisma.recordVersion.findFirst({
   ...
   */
  async restoreVersion(versionId: string, createdBy: string) {
    const oldVersion = await prisma.recordVersion.findUnique({
      where: { id: versionId },
    });
  ...
  async diffVersions(versionIdA: string, versionIdB: string) {
    const [a, b] = await Promise.all([
      prisma.recordVersion.findUnique({ where: { id: versionIdA } }),
      prisma.recordVersion.findUnique({ where: { id: versionIdB } }),
    ]);
