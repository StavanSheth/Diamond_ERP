...
interface SaveDraftRevisionPayload {
  payload: Record<string, unknown>;
...
export class DraftService {
   ...
   */
  private async _nextDraftNumber(): Promise<string> {
    const lastDraft = await prisma.documentDraft.findFirst({
      orderBy: { createdAt: 'desc' },
      select: { draftNumber: true },
    });
    if (!lastDraft) return 'DR-000001';
   ...
   */
  async createDraft(params: CreateDraftPayload) {
   ...
   */
  async saveDraftRevision(draftId: string, params: SaveDraftRevisionPayload) {
   ...
   */
  async commitDraft(draftId: string, createdBy: string) {
   ...
   */
  async abandonDraft(draftId: string) {
   ...
   */
  async listDrafts(filters?: { status?: string; entityType?: string; createdBy?: string }) {
